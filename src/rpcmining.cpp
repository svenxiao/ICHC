...
Value getposinfo(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 0)
        throw runtime_error(
            "getposinfo\n"
    ...
    obj.push_back(Pair("enabled", GetBoolArg("-genpos", false)));
...
Value checkkernel(const Array& params, bool fHelp)
{
    ...
    if (vNodes.empty())
        throw JSONRPCError(-9, "ICH-C is not connected!");

    if (IsInitialBlockDownload())
        throw JSONRPCError(-10, "ICH-C is downloading blocks...");
...
Value getwork(const Array& params, bool fHelp)
{
    ...
    if (vNodes.empty())
        throw JSONRPCError(RPC_CLIENT_NOT_CONNECTED, "ICH-C is not connected!");

    if (IsInitialBlockDownload())
        throw JSONRPCError(RPC_CLIENT_IN_INITIAL_DOWNLOAD, "ICH-C is downloading blocks...");
