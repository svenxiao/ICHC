...
#if defined(NDEBUG)
# error "ICH-C cannot be compiled without assertions."
#endif
...
const string strMessageMagic = "ICH-C Signed Message:\n";
                        ...
                        bool* pfMissingInputs)
{
    ...
    // ICH-C: coinstake is also only valid in a block, not as a loose transaction
    if (tx.IsCoinStake())
...
// ICH-C: find block wanted by given orphan block
...
// ICH-C: find last block index up to pindex
...
unsigned int GetNextTargetRequired(const CBlockIndex* pindexLast, bool fProofOfStake)
{
    ...
    // ICH-C: target change every block
    // ICH-C: retarget with exponential moving toward target spacing
    CBigNum bnNew;
                                 ...
                                 const CBlockIndex* pindexBlock, bool fBlock, bool fMiner, unsigned int flags)
{
    ...
    // fMiner is true when called from the internal ICH-C miner
    ...
    if (!IsCoinBase())
    {
        ...
        for (unsigned int i = 0; i < vin.size(); i++)
        {
            ...
            // ICH-C: check transaction timestamp
...
bool CBlock::DisconnectBlock(CTxDB& txdb, CBlockIndex* pindex)
{
    ...
    // ICH-C: clean up wallet after disconnecting coinstake
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex, bool fJustCheck)
{
    ...
    if (IsProofOfStake())
    {
        // ICH-C: coin stake tx earns reward instead of paying fee
        uint64_t nCoinAge;
    ...
    // ICH-C: track money supply and mint amount info
...
// ICH-C: total coin age spent in transaction, in the unit of coin-days.
...
bool CBlock::AddToBlockIndex(unsigned int nFile, unsigned int nBlockPos, const uint256& hashProof)
{
    ...
    // ICH-C: compute chain trust score
    pindexNew->nChainTrust = (pindexNew->pprev ? pindexNew->pprev->nChainTrust : 0) + pindexNew->GetBlockTrust();

    // ICH-C: compute stake entropy bit for stake modifier
...
bool ProcessBlock(CNode* pfrom, CBlock* pblock)
{
    ...
    if (!mapBlockIndex.count(pblock->hashPrevBlock))
    {
        ...
        if (pfrom) {
            ...
            // ICH-C: getblocks may not obtain the ancestor block rejected
...
#ifdef ENABLE_WALLET
// ICH-C: attempt to generate suitable proof-of-stake
