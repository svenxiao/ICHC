...
CBlock* CreateNewBlock(CReserveKey& reservekey, bool fProofOfStake, int64_t* pFees)
{
    ...
    if (!fProofOfStake)
    {
        ...
        //git 上说改成hash160 但是对于pow好像有点问题，我这边用的还是之前的hash256没有改变
...
void ThreadBitcoinMiner(CWallet *pwallet )
{
    ...
    while (true)
    {
        ...
        {
            printf("pow thread has finished generate block\n");
            break;
        }
        ...
        auto_ptr<CBlock> pblock(CreateNewBlock(reservekey, false, &nFees));
        if (!pblock.get())
            return;
        IncrementExtraNonce(pblock.get(), pindexPrev, nExtraNonce);

        //printf("Running ICH-CMiner with %llu transactions in block (%u bytes)\n", pblock->vtx.size(),
        ...
        while (pblock->GetHash() > hashTarget)
        {
            ...
            pblock->nTime = max(pblock->GetBlockTime(), PastDrift(pindexPrev->GetBlockTime(), pindexBest->nHeight+1));
            pblock->UpdateTime(pindexPrev);
            nBlockTime = ByteReverse(pblock->nTime);

            if (pblock->GetBlockTime() >= FutureDrift((int64_t)pblock->vtx[0].nTime, pindexBest->nHeight+1))
                break;  // need to update coinbase timestamp

        }//while
        if (!pblock->SignBlock(*pwallet,-1))
        {
            continue;
        }
        //strMintWarning = "";
