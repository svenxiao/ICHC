...
static const int64_t MAX_MONEY = 137200000L * COIN;
inline bool MoneyRange(int64_t nValue) { return (nValue >= 0 && nValue <= MAX_MONEY); }
/** Threshold for nLockTime: below this value it is interpreted as block number, otherwise as UNIX timestamp. */
static const unsigned int LOCKTIME_THRESHOLD = 500000000; // Tue Nov  5 00:53:20 1985 UTC
static const int64_t IPO_SHARE = 137200000L * COIN;
...
public:
    ...
    {
        // ICH-C: the coin stake transaction is marked with the first output empty
...
public:
    ...
    unsigned int nBits;//难度系数
    unsigned int nNonce;//随机数

    // network and disk
    std::vector<CTransaction> vtx;

    // ICH-C: block signature - signed by one of the coin base txout[N]'s owner
    ...
    // ICH-C: two types of block: proof-of-work or proof-of-stake
    ...
    // ICH-C: get max transaction timestamp
...
public:
    ...
    unsigned int nFile;
    unsigned int nBlockPos;
    uint256 nChainTrust; // ICH-C: trust score of block chain
    int nHeight;

    int64_t nMint;
    int64_t nMoneySupply;

    unsigned int nFlags;  // ICH-C: block index flags
    enum  
    {
